...
if(isset($_REQUEST["name"]) and !empty($_REQUEST["name"]) and isset($_REQUEST["mno"]) and !empty($_REQUEST["mno"]) and isset($_REQUEST["email"]) and !empty($_REQUEST["email"])  and isset($_REQUEST["city"]) and !empty($_REQUEST["city"]) and isset($_REQUEST["pincode"]) and !empty($_REQUEST["pincode"]) and isset($_REQUEST["Password"]) and !empty($_REQUEST["Password"]) and isset($_REQUEST["confirmPassword"]) and !empty($_REQUEST["confirmPassword"])) {
    ...
    if ($flag == 1) {
                ...
                <div class="w3-container w3-padding">
                    ...
                    <div class="w3-center">
                        <button class="w3-button w3-indigo w3-hover-indigo w3-large"
                                onclick="javascript:location.href='adminuploader.php'">Back
                        </button>
                    </div>
                </div>
            </div>
        ...
        if (mysqli_num_rows($checkUs) > 0) {
            ...
            $saveUs = mysqli_query($conn, "insert into users (name,phone,email,city,pincode,password,status,type) values ('" . $_REQUEST["name"] . "','" . $_REQUEST["mno"] . "','" . $_REQUEST["email"] . "','" . $_REQUEST["city"] . "','" . $_REQUEST["pincode"] . "','" . $_REQUEST["Password"] . "','active','uploader')");
            if ($saveUs) {
                ?>
                <div class="w3-modal" id="errorMsg" style="display: block">
                    <div class="w3-modal-content w3-display-middle w3-mobile">
    ...
    <div class="w3-modal" id="errorMsg" style="display: block">
        <div class="w3-modal-content w3-display-middle w3-mobile">
            <div class="w3-container w3-padding">
                ...
                <div class="w3-center">
                    <button class="w3-button w3-indigo w3-hover-indigo w3-large" onclick="javascript:location.href='adminuploader.php.php'">Back</button>
                </div>
            </div>
        </div>
    </div>
